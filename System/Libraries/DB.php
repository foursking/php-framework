...
defined('DATABASE_LOG_SQL_TIME') || define('DATABASE_LOG_SQL_TIME', 0.2);
...
class DB
{
    ...
    protected $auto_close_mysql_link = false; // true 自动关闭mysql连接 false 不自动关闭
    /**
     * @var AbstractLogger
     */
    ...
    protected function connect()
    {
        ...
        return self::ConnectionWapper($this->_config);
     ...
     *
     */
    protected function ConnectionWapper($db_config)
    {

        $db_key = md5($db_config['database'] . '_' . $db_config['host'] . '_' . $db_config['port']);

        if (isset(self::$connections[$db_key]) && self::$connections[$db_key] instanceof \PDO) {
            return self::$connections[$db_key];
        }

        try {
            $time_start       = microtime(1);
            self::$connections[$db_key] = new \PDO(
                "mysql:host=" . $db_config['host'] . ";dbname=" . $db_config['database'] . ";port=" . $db_config['port'],
                $db_config['user'],
                $db_config['pass'],
                array(
                    \PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'",
         ...
         $this->logDebug("[T] connect info:" . "mysql:host=" . $db_config['host'] . ";dbname=" . $db_config['database'] . ";port=" . $db_config['port']);
    ...
    {
        if ($message) {
            @$message = sprintf('%s %s' ,  $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'] ,$message);
            if ($this->external_logger) {
